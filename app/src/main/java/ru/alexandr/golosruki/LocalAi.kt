...
object LocalAi {
    ...
    private val planVerbs = listOf("ОТКРОЙ", "НАПИШИ", "НАЖМИ", "ВВОД", "ПОЗВОНИ", "НАЗАД", "ДОМОЙ")

    /** Агент Фаза 2: выбрать ОДНО следующее действие по текущему экрану. */
    fun nextAction(ctx: Context, goal: String, screen: String, history: List<String>): String {
        ensureEngine(ctx)
        if (!engine.isReady()) return "ГОТОВО"
        val sys = """Ты управляешь телефоном шаг за шагом, чтобы достичь цели пользователя.
Тебе дают ЦЕЛЬ, список видимых элементов экрана с номерами и уже сделанные шаги.
Выбери РОВНО ОДНО следующее действие. Ответь только им, без пояснений:
ОТКРОЙ <название приложения> — открыть приложение. Пиши НАЗВАНИЕ (например: ОТКРОЙ телеграм), НЕ номер.
НАЖМИ <номер> — нажать элемент списка по его номеру.
ВПИШИ <номер> <текст> — ввести текст в поле с этим номером (номер обязателен).
ВВОД — отправить.
НАЗАД
ГОТОВО — только если цель ТОЧНО достигнута.
Правила:
- Если нужного приложения ещё нет на экране (смотри строку «Приложение:») — первым действием ОТКРОЙ <название>.
- Для НАЖМИ и ВПИШИ бери номер строго из списка на экране.
- Не повторяй уже сделанный шаг и не пиши ГОТОВО, пока задача не выполнена."""
        val hist = if (history.isEmpty()) "пока ничего" else history.takeLast(4).joinToString("; ")
        val user = "ЦЕЛЬ: $goal\nЭкран:\n$screen\nСделано: $hist\nСледующее действие:"
        val raw = engine.generate(sys, user).trim()
        return raw.split("\n").map { it.trim() }.firstOrNull { it.isNotBlank() } ?: "ГОТОВО"
    }

    /** Агент: превратить свободную просьбу в короткий план (Фаза 1, запас). */
    fun plan(ctx: Context, intent: String): List<String> {
        ensureEngine(ctx)
        if (!engine.isReady()) return emptyList()
        val sys = """Ты — планировщик действий на телефоне. Преврати просьбу пользователя в короткий список шагов.
Используй ТОЛЬКО эти команды, по одной на строке, без пояснений, без нумерации:
ОТКРОЙ <приложение>
НАПИШИ <текст>
НАЖМИ <надпись на кнопке>
ВВОД
ПОЗВОНИ <имя контакта>
НАЗАД
ДОМОЙ
Максимум 4 шага. Выводи только команды.
Пример. Просьба: позвони маме
ПОЗВОНИ мама
Пример. Просьба: открой ютуб
ОТКРОЙ ютуб
Пример. Просьба: напиши жене в телеграм что задержусь
ОТКРОЙ телеграм
НАЖМИ жена
НАПИШИ задержусь
ВВОД"""
        val raw = engine.generate(sys, "Просьба: $intent")
        return raw.split("\n")
            .map { it.trim().trimStart('-', '*', '•', '1', '2', '3', '4', '.', ')', ' ').trim() }
            .filter { line -> planVerbs.any { line.uppercase().startsWith(it) } }
            .take(5)
    }
