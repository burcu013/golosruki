...
object LocalAi {
    ...
    fun answer(ctx: Context, ask: Boolean, userText: String): String {
        ...
        if (ask) {
            sys = buildString {
                append("Ты — голосовой помощник в приложении управления смартфоном. Отвечай на русском языке кратко и по существу. ")
                if (profile.name.isNotBlank()) append("Обращайся к пользователю по имени: ${profile.name}. ")
                if (profile.mode == "aid") append("Пользователь управляет телефоном только голосом (ограниченная подвижность). ")
                if (profile.capabilities.contains(AiProfile.CAP_SPEECH_ONLY)) {
                    append("КРИТИЧНО: пользователь НЕ может действовать руками или телом — только речь и телефон. ")
                    append("НИКОГДА не предлагай физических действий («возьми», «сходи», «принеси», «нажми рукой», «выпей»). ")
                    append("Вместо этого предлагай попросить близких/окружающих или сделать через телефон голосом. ")
                }
                if (profile.capabilities.contains(AiProfile.CAP_LOW_VISION)) append("У пользователя слабое зрение: объясняй словами, не ссылайся на мелкий текст, цвет, картинки. ")
                if (profile.capabilities.contains(AiProfile.CAP_LOW_HEARING)) append("Пользователь плохо слышит: формулируй ключевое чётко и коротко. ")
                if (profile.notes.isNotBlank()) append("Учитывай особенности: ${profile.notes}. ")
                append("Дай прямой ответ на вопрос. НЕ задавай встречных вопросов, НЕ здоровайся, не представляйся. ")
                append("Если это пример — посчитай и дай число. Для расчёта в несколько действий считай по шагам, затем дай итог; простое считай сразу. ")
