        ...
        if (CloudAi.isConfigured(ctx) && Net.isOnline(ctx)) {
            ...
            if (ask) {
                ...
                sysC = buildString {
                    ...
                    append("ВАЖНО: ответ зачитывается вслух синтезатором. Пиши как живой человек говорит: ")
                    append("без markdown, без звёздочек, решёток и заголовков; без нумерованных и маркированных списков — если нужно перечислить, делай это в речи словами («во-первых… во-вторых…» или просто через запятую), а не цифрами с точкой. ")
                    append("Управляй интонацией знаками препинания: запятые и тире — для коротких пауз и дыхания, многоточие… — для задумчивости, восклицательный и вопросительный знаки — для эмоции. ")
                    append("Длинные мысли разбивай на короткие предложения, чтобы звучало естественно, а не сплошным потоком. ")
